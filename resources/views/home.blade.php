        ...
        <div class="card-body">
          <form class="was-validated" action="{{asset('home')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
              <select class="custom-select" id='catalog_id'name="category_id" required>
                <option value="1" selected="selected">Смартфоны</option>
          ...
          <table class="table table-bordered table-striped" width="100%">
            ...
            <tr>
              <th>
                <img src="/uploads/{{$p->user_id}}/ss_{{$p->picture}}" alt="">
              ...
              <th>{{$p->body}}</th>
              <th>{{$p->price}}</th>
              <th><a href="#" class="btn btn-block btn-primary">Редактировать</a> <br />
                <a href="{{asset('product/delete/'.$p->id)}}" class="btn btn-block btn-primary">
                  Удалить</a>
                <a href="#" class="btn btn-block btn-primary show-modal" data-id="{{$p->id}}">Просмотр</a>
              </th>
            </tr>
            @endforeach
          </table>
